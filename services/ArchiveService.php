...
use YesWiki\Core\Service\ArchiveService as CoreArchiveService;
...
public function archiveNew(
    ...
    try {
        ...
        $this->createZipNew($location, $foldersToInclude, $blacklistedRootFolders , $output, $sqlContent, $onlyDb, $hideConfigValuesParams, $inputFile, $outputFile);
        if (!file_exists($location)) {
 ...
 * @return string uid
 */
public function startArchiveNew(
    bool $savefiles = true,
    bool $savedatabase = true,
    array $foldersToInclude = [],
    array $foldersToExclude = [],
    ...
    if ($callAsync) {
        ...
        $output = "";
        $location = $this->archiveNew($output, $savefiles, $savedatabase, $foldersToInclude, $foldersToExclude, null, $uidData['uid']);
        if (empty($location)) {
            $this->cleanUID($uidData['uid'], $privatePath);
            return '';
        } else {
 ...
 * @param array $foldersToInclude
    ...
    */
    protected function createZipNew(
        string $zipPath,
        array $foldersToInclude,
        ...
        if (!$onlyDb) {
            ...
            while (count($dirs)) {
                ...
                if (empty($baseDirName) || (!empty($baseDirName) && $this->shouldIncludeFolderNew($baseDirName, $whitelistedRootFolders,$blacklistedRootFolders))){
                    ...
                    while (false !== ($file = readdir($dh))) {
                        if ($file != '.' && $file != '..') {
                            ...
                            if (empty($baseDirName) && $file == "wakka.config.php") {
                                ...
                                if ($this->shouldIncludeFolderNew($relativeName, $whitelistedRootFolders,$blacklistedRootFolders)) {
     ...
     * @return bool
     */
    protected function shouldIncludeFolderNew(
